 ...
 *******************************************************************************/
package org.crossbridge.jst.server.report.ui.internal.editor;

import java.beans.PropertyChangeEvent;
import java.beans.PropertyChangeListener;

import org.crossbridge.jst.server.report.core.internal.ITomcatServer;
import org.crossbridge.jst.server.report.core.internal.ITomcatVersionHandler;
import org.crossbridge.jst.server.report.core.internal.TomcatServerReport;
import org.crossbridge.jst.server.report.core.internal.command.SetDebugModeCommand;
import org.crossbridge.jst.server.report.core.internal.command.SetModulesReloadableByDefaultCommand;
import org.crossbridge.jst.server.report.core.internal.command.SetSaveSeparateContextFilesCommand;
import org.crossbridge.jst.server.report.core.internal.command.SetSecureCommand;
import org.crossbridge.jst.server.report.core.internal.command.SetServeModulesWithoutPublishCommand;
import org.crossbridge.jst.server.report.ui.internal.ContextIds;
import org.crossbridge.jst.server.report.ui.internal.Messages;
import org.crossbridge.jst.server.report.ui.internal.TomcatUIPlugin;
...
public class ServerGeneralEditorSection extends ServerEditorSection {
	protected TomcatServerReport tomcatServer;
		...
		listener = new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent event) {
				if (updating)
					return;
				updating = true;
				if (TomcatServerReport.PROPERTY_SECURE.equals(event.getPropertyName())) {
					Boolean b = (Boolean) event.getNewValue();
					ServerGeneralEditorSection.this.secure.setSelection(b.booleanValue());
				} else if (TomcatServerReport.PROPERTY_DEBUG.equals(event.getPropertyName())) {
					Boolean b = (Boolean) event.getNewValue();
...
//		whs.setHelp(reloadableByDefault, ContextIds.SERVER_EDITOR_SECURE);
	...
	public void init(IEditorSite site, IEditorInput input) {
		...
		if (server != null) {
			tomcatServer = (TomcatServerReport) server.loadAdapter(TomcatServerReport.class, null);
			addChangeListener();
		}
		initialize();
	}
